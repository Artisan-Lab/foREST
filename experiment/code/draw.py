...
restler_rw_file = open('../data/gitlab-project-branch-commit/RESTCT_commit.csv')
...
for _ in range(0, evomaster_length):
    ...
    y22.append(float(evomaster_data[_][1].strip('%'))/100)
...
for _ in range(0, restler_length):
    ...
    y32.append(float(restler_data[_][1].strip('%'))/100)
...
plt.title('commit')
plt.legend(bbox_to_anchor=(1, 0.2), loc='upper right', borderaxespad=0, fontsize=8)
fig.savefig('WordPress.png', format='png', dpi=300, bbox_inches='tight')
