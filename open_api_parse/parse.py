    ...
    for object in objects:
        ...
        if type == 'object':
            if 'properties' in objects[object]:
                object_feild = field_info(object,type,object_handle(objects[object]['properties']))
            else:
                object_feild = field_info(object,type,None)
        elif type == 'array':
            object_feild = field_info(object,type,array_handle(objects[object]['items']))
        else:
            object_feild = field_info(object,type,None)
...
def get_api_info(path):
    try:
        json_string = open(path, 'r', encoding='utf-8').read()
        spec = json.loads(json_string)
        resolve_circular_references(spec)
