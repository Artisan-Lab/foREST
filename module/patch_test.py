...
def one_fuzz_k_times(k, parameter, api_id, cov_url, ini_coverage_rate_executed_code,lll):
    for i in range(k):
        ...
        print('fuzz  ' + str(api_id) + ' patch ' + url)
        ...
        if response != None:
            ...
            if now_coverage_rate_executed_code != ini_coverage_rate_executed_code:
                ...
                if lll == 0:
                    save_patch_fuzz_success(fuzz_success_json_data)
                elif lll == 1:
                    save_patch_fuzz_optional_success(fuzz_success_json_data)
...
def patch_fuzz_test(k, api_info, cov_url):
    lll = 0
...
def patch_fuzz_test_optional(k, api_info, cov_url):
    lll = 1
    ini_coverage_rate_executed_code = GetCoverage.getCoverage_rate_executed_code(cov_url)
    api_id = api_info.api_id
    global url
    ...
    fuzz_success_data = {}
    fuzz_success_data['id'] = api_info.api_id
    fuzz_success_json_data = fuzz_success.lindex('patch_success!!!!!!',-1)
...
def save_patch_fuzz_success(fuzz_success_json_data):
    fuzz_success.lpush('patch_success!!!!!!', fuzz_success_json_data)

def save_patch_fuzz_optional_success(fuzz_success_json_data):
    fuzz_success.lpush('patch_optional_success!!!!!!', fuzz_success_json_data)
