...
# 一个参数k次覆盖率不变，则结束fuzz
def one_fuzz_k_times(k, parameter, api_id, cov_url, ini_coverage_rate_executed_code,lll):
    for i in range(k):
        p_name = parameter.keys()
        p_type = parameter[p_name][:-1]
        p_location = parameter[p_name][-1]
        value_fuzz = fuzz(p_type)
        url, headers, data, fuzz_success_data = make(url, p_location, p_name, value_fuzz)
        print('fuzz  ' + str(api_id) + ' delete ' + url)
        ...
        if response != None:
            ...
            if now_coverage_rate_executed_code != ini_coverage_rate_executed_code:
                ...
                if lll == 0:
                    save_delete_fuzz_success(fuzz_success_json_data)
                elif lll == 1:
                    save_delete_fuzz_optional_success(fuzz_success_json_data)
                fuzz_success_data.clear()
                one_fuzz_k_times(k, parameter, api_id, cov_url, ini_coverage_rate_executed_code)
        else:
            pass
        print(response.json())

# 多个参数k次覆盖率不变，则结束fuzz
def mult_fuzz_k_times(k,c,pa_location,pa_name,value_fuzz ,parameter, api_id, cov_url, ini_coverage_rate_executed_code,lll):
    for i in range(k):
        for pa in c:
        ...
        global url
        url, headers, data, fuzz_success_data = make(url, pa_location, pa_name, value_fuzz)
        '''Redis存储delete过的api_id'''
        delete.lpush('api_id_p', api_id)
        # '''配置token'''
        # if '?' in url:
        ...
        if response != None:
            ...
            if now_coverage_rate_executed_code != ini_coverage_rate_executed_code:
                ...
                if lll == 0:
                    ...
                    save_delete_fuzz_optional_success(fuzz_success_json_data)
                fuzz_success_data.clear()
                mult_fuzz_k_times(k,c,pa_location,pa_name,value_fuzz ,parameter, api_id, cov_url, ini_coverage_rate_executed_code)
        else:
            pass
        print(response.json())

def delete_fuzz_test(k, api_info, cov_url):
    lll = 0
    ...
    if length == 1:
        one_fuzz_k_times(k, parameter, api_id, cov_url, ini_coverage_rate_executed_code,lll)
    else:
        ...
        for b in a:
            ...
            for pa in g:  # 不变参数确定，反复改变可变参数
                ...
                pa_type = parameter[pa][:-1]
                value_fuzz.append(fuzz(pa_type))
            url, headers, data, fuzz_success_data = make(url, pa_location, pa_name, value_fuzz)    # 不变参数构造完成url
            mult_fuzz_k_times(k, c, pa_location, pa_name, value_fuzz, parameter, api_id, cov_url,
                              ini_coverage_rate_executed_code,lll)

def delete_fuzz_test_optional(k, api_info, cov_url):
    lll = 1
    ini_coverage_rate_executed_code = GetCoverage.getCoverage_rate_executed_code(cov_url)
    api_id = api_info.api_id
    global url
    ...
    fuzz_success_data = {}
    fuzz_success_data['id'] = api_info.api_id
    fuzz_success_json_data = fuzz_success.lindex('delete_success!!!!!!',-1)
    ...
    for optional_param in end_list:
        ...
        if optional_param == None:
            ...
            if length == 1:
                one_fuzz_k_times(k, optional_param, api_id, cov_url, ini_coverage_rate_executed_code,lll)
            else:
                a = Combination.get_combine(Combination, optional_param.keys())
                for b in a:
                    c = list(b)  # c是要改变的参数list
                    d = set(optional_param.keys())
                    e = set(c)
                    f = d ^ e
                    g = list(f)  # g是不改变的参数list
                    pa_location = []
                    ...
                    for pa in g:  # 不变参数确定，反复改变可变参数
                        ...
                        value_fuzz.append(fuzz(pa_type))
                    url, headers, data, fuzz_success_data = make(url, pa_location, pa_name, value_fuzz)  # 不变参数构造完成url
                    mult_fuzz_k_times(k, c, pa_location, pa_name, value_fuzz, optional_param, api_id, cov_url,
                                      ini_coverage_rate_executed_code,lll)

def save_delete_fuzz_success(fuzz_success_json_data):
    fuzz_success.lpush('delete_success!!!!!!', fuzz_success_json_data)

def save_delete_fuzz_optional_success(fuzz_success_json_data):
    fuzz_success.lpush('delete_optional_success!!!!!!', fuzz_success_json_data)
