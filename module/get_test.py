...
def one_fuzz_k_times(fuzz_success_data, k, parameter, api_id, cov_url, ini_coverage_rate_executed_code,lll):
    for i in range(k):
        p_name = str(parameter.keys())
        p_type = list(parameter[p_name])[:-1]
        p_location = list(parameter[p_name])[-1]
        ...
        if response != None:
            ...
            if now_coverage_rate_executed_code != ini_coverage_rate_executed_code:
                ...
                if lll == 0:
                    save_get_fuzz_success(fuzz_success_json_data)
                elif lll == 1:
                    save_get_fuzz_optional_success(fuzz_success_json_data)
...
def mult_fuzz_k_times(fuzz_success_data, k,c,pa_location,pa_name,value_fuzz ,parameter, api_id, cov_url, ini_coverage_rate_executed_code,lll):
    parameter_trans = {}
    for b in parameter:
        parameter_trans[list(b.keys())[0]] = list(b.values())[0]
    for i in range(k):
        for pa in c:
            pa_location.append(parameter_trans[pa][-1])
            pa_name.append(pa)
            pa_type = parameter_trans[pa][:-1]
...
def get_fuzz_test(k, api_info, cov_url):
    lll = 0
...
def get_fuzz_test_optional(k, api_info, cov_url):
    lll = 1
    ini_coverage_rate_executed_code = GetCoverage.getCoverage_rate_executed_code(cov_url)
    api_id = api_info.api_id
    global url
    ...
    fuzz_success_data = {}
    fuzz_success_data['id'] = api_info.api_id
    fuzz_success_json_data = fuzz_success.lindex('get_success!!!!!!',-1)
    ...
    for optional_param in end_list:
        ...
        if optional_param == None:
            ...
            if length == 1:
                ...
                a = Combination.get_combine(Combination, optional_param)
                for b in a:
...
def save_get_fuzz_success(fuzz_success_json_data):
    fuzz_success.lpush('get_success!!!!!!', fuzz_success_json_data)

def save_get_fuzz_optional_success(fuzz_success_json_data):
    fuzz_success.lpush('get_optional_success!!!!!!', fuzz_success_json_data)
