...
def one_fuzz_k_times(fuzz_success_data, k, parameter,url , api_id, cov_url, ini_coverage_rate_executed_code,lll):
    for i in range(k):
        for i in parameter.keys():
            ...
            print('fuzz  ' + str(api_id) + ' put ' + urll)
            ...
            if response != None:
                ...
                if now_coverage_rate_executed_code != ini_coverage_rate_executed_code:
                    ...
                    if lll == 0:
                        save_put_fuzz_success(fuzz_success_json_data)
                    elif lll == 1:
                        save_put_fuzz_optional_success(fuzz_success_json_data)
...
def mult_fuzz_k_times(fuzz_success_data, k, d, url, headers, data, api_id, cov_url, ini_coverage_rate_executed_code,lll):
    for i in range(k):
        ...
        # else:
        #     url = url + "?private_token=vE9ggmw9HjEwuisyH2eF"
        print('fuzz  ' + str(api_id) + ' put ' + url)
        response = requests.put(url=url, headers=header, data=dat)
        if response != None:
            response_json = response.json()
            json_txt(response_json)
...
def put_fuzz_test(k, api_info, cov_url):
    print("必选参数")
    lll = 0
    print(api_info)
...
def put_fuzz_test_optional(k, api_info, cov_url, conf_k):
    print("可选参数")
    lll = 1
    ini_coverage_rate_executed_code = GetCoverage.getCoverage_rate_executed_code(cov_url)
    print(ini_coverage_rate_executed_code)
    ...
    fuzz_success_json_data = fuzz_success.lindex('put_success!!!!!!',-1)
...
def save_put_fuzz_success(fuzz_success_json_data):
    fuzz_success.lpush('put_success!!!!!!', fuzz_success_json_data)

def save_put_fuzz_optional_success(fuzz_success_json_data):
    fuzz_success.lpush('put_optional_success!!!!!!', fuzz_success_json_data)
